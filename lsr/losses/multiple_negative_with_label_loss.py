...
class MultipleNegativeWithLabelLoss(Loss):
    """
    The MultipleNegativeWithLabelLoss implements the CrossEntropyLoss underneath. There are one positive document and multiple negative documents per query.
    ...
    def __init__(self, q_regularizer=None, d_regularizer=None, q_cls_weights=None, d_cls_weights=None) -> None:
        """
        Constructing MultipleNegativeWithLabelLoss
        Parameters
        ----------
        ...
        """
        super(MultipleNegativeWithLabelLoss, self).__init__(q_regularizer, d_regularizer)
        self.q_cls_weights = [float(x) for x in q_cls_weights.split(',')]
        self.d_cls_weights = [float(x) for x in d_cls_weights.split(',')]
        self.ce = nn.CrossEntropyLoss(reduction="mean")

    def forward(self, q_reps, d_reps, q_logits, d_logits, q_labels, d_labels):
        """
        Calculating the MultipleNegativeWithLabelLoss over a batch of query and document
        Parameters
        ----------
        q_reps: torch.Tensor
        ...
        q_cls_loss = 0
        q_acc = []
        for idx, (logits, labels, weight) in enumerate(zip(q_logits, q_labels, self.q_cls_weights)):
            q_cls_loss += weight * self.ce(logits, labels)
            preds = torch.argmax(logits, dim=1)
            acc = (preds == labels).float().sum()
            q_acc.append((acc, len(preds)))

        d_cls_loss = 0
        d_acc = []
        for logits, labels, weight in zip(d_logits, d_labels, self.d_cls_weights):
            d_cls_loss += weight * self.ce(logits, labels)
            preds = torch.argmax(logits, dim=1)
            acc = (preds == labels).float().sum()
            d_acc.append((acc, len(preds)))
        acc = [(q[0]+d[0]) / (q[1] + d[1]) for q, d in zip(q_acc, d_acc)]

        loss = ce_loss + q_cls_loss + d_cls_loss
        ...
        to_log = {
            ...
            "mn_loss": ce_loss.detach(),
            "q_cls_loss": q_cls_loss.detach(),
            "d_cls_loss": d_cls_loss.detach(),
            "acc_root_category": acc[0],
            "acc_category": acc[1],
        }
